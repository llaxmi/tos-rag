...
- **Retrieval scope = whole corpus** (both docs per query), k=5 (amended 2026-07-23; was 8), cosine distance, **exact scan** (no HNSW/IVFFlat — perfect recall, deterministic, trivial at this scale), stable tiebreak `ORDER BY embedding <=> q, id`.
...
7. **Experimental design** — 15 configs (5 strategies × 128/256/512 gemma-tokens), constants (k=5, prompt, temp 0, seed, overlap 0, embedder), Phase 1 (Llama-fast, 20 questions), Phase 2 (best config, both generators, 30 questions, held-out 10 reported separately).
